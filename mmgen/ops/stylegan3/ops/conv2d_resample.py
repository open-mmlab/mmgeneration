...
def _conv2d_wrapper(x,
    ...
    # Note: conv2d() actually performs correlation (flip_weight=True) not
    # convolution (flip_weight=False).
    if not flip_weight and (kw > 1 or kh > 1):
        w = w.flip([2, 3])

    # Execute using conv2d_gradfix.
    op = conv2d_gradfix.conv_transpose2d if transpose else (
...
def conv2d_resample(x,
    ...
    # Fast path: upsampling with optional downsampling => use transpose
    # strided convolution.
    if up > 1:
        if groups == 1:
            w = w.transpose(0, 1)
