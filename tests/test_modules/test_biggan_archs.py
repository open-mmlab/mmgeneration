...
class TestSpectralNorm:
    ...
    def test_spectral_norm(self):
        sn = SpectralNorm(**self.sn_kwargs)
        sn.register_buffer('weight', torch.randn((1,1,3,3)))
        assert len(sn.u) == 1
        assert sn.u[0].shape == (1,1)
        assert len(sn.sv) == 1
        assert sn.sn_weight().shape == (1, 1, 3, 3) 
        
        # test with different num_svs
        cfg = deepcopy(self.sn_kwargs)
        cfg['num_svs'] = 3
        sn = SpectralNorm(**cfg)
        sn.register_buffer('weight', torch.randn((1,1,3,3)))
        assert len(sn.u) == 3
        ...
        # test with different num_outputs
        cfg = deepcopy(self.sn_kwargs)
        cfg['num_outputs'] = 3
        sn = SpectralNorm(**cfg)
        sn.register_buffer('weight', torch.randn((3,1,3,3)))
        assert len(sn.u) == 1
        assert sn.u[0].shape == (1,3)
        assert len(sn.sv) == 1
        assert sn.sn_weight().shape == (3, 1, 3, 3) 
